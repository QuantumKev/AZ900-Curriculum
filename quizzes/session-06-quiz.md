...
| 5 | B | 2.3.3, 2.1.2 (carry-forward) | ZRS replicates synchronously across three availability zones in the primary region and keeps data readable and writable if one zone is unavailable. LRS keeps all three copies in one datacenter. Answering this requires recalling what an availability zone is. | [Azure storage redundancy](https://learn.microsoft.com/en-us/training/modules/describe-azure-storage-services/3-redundancy) · [Azure physical infrastructure](https://learn.microsoft.com/en-us/training/modules/describe-core-architectural-components-of-azure/5-describe-azure-physical-infrastructure) |
| 6 | D | 2.3.3, 1.2.2 (carry-forward) | GZRS combines zone redundancy in the primary region with geo-replication to the paired secondary region, and is Microsoft's recommendation for maximum availability and disaster-recovery resilience. GRS uses LRS in both regions, so it does not protect against a zone failure the same way. This is reliability as a cloud benefit made concrete. | [Azure storage redundancy](https://learn.microsoft.com/en-us/training/modules/describe-azure-storage-services/3-redundancy) · [Reliability and predictability](https://learn.microsoft.com/en-us/training/modules/describe-benefits-use-cloud-services/3-reliability-predictability-cloud) |
...
**On the carry-forward design here:** items 5 and 6 carry 2.1.2 and 1.2.2 inside new
storage content rather than as separate stand-alone items. Redundancy cannot be reasoned
about without knowing what an availability zone is, so the carry-forward is embedded. Call
the link out explicitly during the review — learners who picked the right answer by
memorizing acronyms should still be asked *why* ZRS survives a zone failure.
